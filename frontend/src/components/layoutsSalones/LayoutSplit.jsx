...
import { TEXTOS_SALONES } from '../../utils/diccionario'; // <--- Importamos diccionario
...
export default function LayoutSplit({ 
    ...
    // Diccionario para etiquetas fijas (ej. "Horario")
        ...
        <div className="flex w-full h-full gap-8 transition-all duration-500">
            ...
            <div className="flex-1 relative rounded-[3rem] overflow-hidden shadow-2xl border border-white/10" style={{ backgroundColor: fondo }}>
                ...
                {!itemActual && (
                    <div className="absolute inset-0 flex items-center justify-center opacity-10">
                        <img src={config?.logo} className="w-1/3 grayscale animate-pulse" alt="Logo" />
                    </div>
                )}
                ...
                {fotosActivas.length > 1 && (
                    <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-20">
                        {fotosActivas.map((_, idx) => (
                            <div key={idx} className={`h-1.5 rounded-full transition-all duration-500 shadow-sm ${idx === indice ? 'w-6' : 'w-1.5 bg-white/30'}`} 
            ...
            <div className="flex-1 relative rounded-[3rem] overflow-hidden shadow-2xl border border-white/5 backdrop-blur-xl flex flex-col items-center justify-center p-12 text-center" 
                ...
                <div className="w-full flex flex-col items-center h-full justify-center relative">
                    ...
                    {/* Título Traducido */}
                    <h1 className="text-5xl lg:text-7xl font-black mb-10 leading-tight drop-shadow-2xl animate-fade-in-up" 
                    ...
                    {cliente && (
                        <div className="mb-14 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                            <span className="inline-block px-8 py-3 rounded-full border border-white/10 text-xl font-bold uppercase tracking-wider shadow-lg transition-all duration-300" 
                    ...
                    <div className="flex flex-col items-center gap-2 mb-10 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                        {/* Etiqueta Horario Traducida */}
                        <span className="text-sm uppercase tracking-widest opacity-60" style={{ color: texto_evento }}>
                            {labelHorario}
                        </span>
                        
                        {/* Fecha Localizada */}
                        <span className="text-sm font-bold uppercase opacity-80" key={`date-${idiomaActual}`}>
                            {fechaFormateada}
                        </span>
                    ...
                    {mensaje && (
                        <div className="w-4/5 mx-auto bg-white/5 p-6 rounded-2xl border border-white/5 animate-fade-in-up" 
                            ...
                            <p className="text-xl font-serif italic leading-relaxed" style={{ color: texto_evento }}>
                                "{mensaje}"
                            </p>
                        </div>
                    )}
                    ...
                    {eventoActual.direccion && (
                        <div className="absolute bottom-0 right-0 p-4 animate-fade-in-up">
