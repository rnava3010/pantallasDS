    ...
    return (
        <div className="h-screen w-screen overflow-hidden relative bg-black text-white">
            ...
            <div className="relative z-10 h-full flex flex-col p-8 justify-between">
                <header className="flex justify-between items-center bg-black/60 p-6 rounded-[2.5rem] border border-white/20 shadow-2xl shrink-0">
                    ...
                    {visibles.map((t, i) => (
                        <div key={i} className="bg-black/60 backdrop-blur-md p-6 rounded-[2rem] flex justify-between items-center border border-white/10 shadow-xl animate-fade-in-up">
                            <div className="max-w-[65%] text-left">
                                <div className="text-xl font-bold uppercase truncate" style={{ textShadow: '2px 2px 4px rgba(0,0,0,1)' }}>
                                    {getTxt(t, 'nombre')}
                                </div>
                                <div className="text-[11px] opacity-40 italic truncate mt-1" style={{ textShadow: '1px 1px 2px rgba(0,0,0,1)' }}>
                ...
                <footer className="flex flex-col gap-5 shrink-0">
                    <div className="bg-black/60 backdrop-blur-md p-5 rounded-[2rem] border border-white/10 shadow-xl text-center">
                    ...
                    <div className="h-12 bg-black/60 backdrop-blur-md rounded-2xl border border-white/10 flex items-center overflow-hidden">
                        <div className="animate-marquee-horizontal whitespace-nowrap">
                            {datos?.avisos.map((aviso, i) => (
                                <span key={i} className="text-sm font-medium tracking-widest uppercase mx-16">
                                    {getTxt(aviso, 'texto')}
                                </span>
                            ))}
                        </div>
