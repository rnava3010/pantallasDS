...
export default function LayoutTarifasHorizontal({ 
    ...
    const t = TEXTOS_TARIFAS[idiomaActual] || TEXTOS_TARIFAS['es']; // Usamos el diccionario
    ...
    return (
        <div className="flex flex-col h-screen w-screen overflow-hidden p-10 transition-all duration-500" style={{ backgroundColor: fondo }}>
            ...
            <header className="flex justify-between items-center mb-8 bg-black/40 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 shadow-2xl">
                ...
                {/* Título Traducido (Usé titulo_largo del diccionario) */}
                <h1 className="text-5xl font-black uppercase tracking-tighter animate-fade-in-up" key={idiomaActual} style={{ color: acento, textShadow: '0 0 20px rgba(0,0,0,0.5)' }}>
                    {t.titulo_largo} 
                </h1>
                    ...
                    return (
                        <div key={i} className="flex justify-between items-center p-5 bg-white/5 rounded-3xl border border-white/5 animate-fade-in-up hover:bg-white/10 transition-colors">
                            <div className="flex flex-col gap-1">
                                <span className="text-3xl font-black uppercase transition-all duration-300" style={{ color: texto_evento }}>{nombre}</span>
                                {descripcion && <span className="text-sm text-white/50 italic font-light transition-all duration-300">{descripcion}</span>}
                            </div>
            ...
            <footer className="h-64 mt-8 grid grid-cols-2 gap-8">
                    ...
                    {clima && (
                        <div className="absolute bottom-6 left-6 flex items-center gap-4 animate-fade-in-up">
                            ...
                            <div>
                                ...
                                <span className="text-[10px] uppercase tracking-widest text-white/80">{t.clima}</span>
                            </div>
                        </div>
                    )}
                </div>

                {/* Banner Vertical + Divisas */}
                <div className="flex flex-col gap-4">
                    <div className="flex-1 bg-black/40 backdrop-blur-md rounded-[2.5rem] border border-white/10 flex items-center justify-center overflow-hidden relative">
                         <div key={bannerTexto} className="animate-marquee-vertical absolute inset-x-8 text-center flex flex-col items-center justify-center min-h-full">
                            <span className="text-2xl font-light tracking-widest text-white uppercase leading-snug">{bannerTexto}</span>
                        </div>
                    </div>

                    {/* Divisas usando el título del diccionario si quisieras, o solo visual */}
                    <div className="h-24 bg-white/5 backdrop-blur-md rounded-[2rem] border border-white/10 flex items-center justify-around px-6">
                        {divisas.length > 0 ? divisas.map((d, idx) => (
                            ...
                            <span className="text-white/30 text-sm uppercase tracking-widest">--</span>
                        )}
                    </div>
                </div>
            </footer>
            ...
            <style>{`
                ...
                @keyframes marqueeVertical { 0% { transform: translateY(100%); opacity: 0; } 10% { opacity: 1; } 90% { opacity: 1; } 100% { transform: translateY(-100%); opacity: 0; } }
