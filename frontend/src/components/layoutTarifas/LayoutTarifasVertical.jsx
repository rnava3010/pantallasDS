...
export default function LayoutTarifasVertical({ 
    ...
    // ✅ Ajustado a 3 items para activar el carrusel si hay más
    ...
    return (
        <div className="h-screen w-screen overflow-hidden p-6 flex flex-col gap-4" style={{ backgroundColor: fondo }}>
            ...
            {/* 1. HEADER REDISEÑADO (Logo Izq | Título Centro | Hora Der) */}
            <header className="grid grid-cols-3 items-center bg-black/40 backdrop-blur-md p-5 rounded-3xl border border-white/10 shadow-xl">
                {/* Izquierda: Logo */}
                ...
                <div className="text-center">
                    <h1 className="text-2xl font-black uppercase leading-tight" style={{ color: acento, textShadow: `0 0 15px ${acento}40` }}>
                        {dict.titulo_largo}
                    </h1>
                </div>
            ...
            {/* 2. LISTADO DE TARIFAS (CARRUSEL) */}
            <main className="flex-1 flex flex-col gap-4 justify-center min-h-0">
                {visibles.map((t, i) => {
                    ...
                    return (
                        <div key={i} className="flex flex-col p-6 bg-white/5 rounded-3xl border border-white/5 animate-fade-in-up shadow-lg">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-2xl font-bold uppercase" style={{ color: texto_evento }}>
                                    {getTxt(t, 'nombre')}
                                </span>
            ...
            {/* 4. FOOTER (GALERÍA Y AVISOS) */}
            <footer className="h-96 flex flex-col gap-4">
                ...
                <div className="h-14 bg-black/60 backdrop-blur-md rounded-2xl border border-white/10 flex items-center overflow-hidden">
                    <div className="animate-marquee-horizontal whitespace-nowrap">
                        {avisosRaw.map((aviso, i) => (
                            <span key={i} className="text-lg font-medium tracking-widest text-white uppercase mx-16">
                                {getTxt(aviso, 'texto')}
                            </span>
                        ))}
                    </div>
            ...
            <style>{`
                .animate-marquee-horizontal { 
                    ...
                    animation: marqueeH 45s linear infinite; 
                }
