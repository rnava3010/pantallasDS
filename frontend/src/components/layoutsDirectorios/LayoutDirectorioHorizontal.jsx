...
export default function LayoutTarifasHorizontal({ 
    ...
    const divisas = datos?.divisas || []; // ✅ Extraemos las divisas
    ...
    return (
        <div className="flex flex-col h-screen w-screen overflow-hidden p-10" style={{ backgroundColor: fondo }}>
            ...
            {/* HEADER */}
            <header className="flex justify-between items-center mb-6 bg-black/40 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 shadow-2xl">
                <img 
                    src={config.logo} 
                    alt="Logo" 
            ...
            <main className="flex-1 flex flex-col gap-3 px-10 mb-4">
                {visibles.map((t, i) => {
                    // Lógica de Precios (Rack vs Promoción)
            ...
            {/* ✅ NUEVA SECCIÓN: BARRA DE DIVISAS */}
            {divisas.length > 0 && (
                <div className="mx-10 mb-6 flex items-center justify-center gap-6 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                    {divisas.map((divisa, idx) => (
                        <div key={idx} className="flex items-center gap-3 bg-black/60 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/10 shadow-lg min-w-[180px]">
                            {/* Bandera */}
                            <span className="text-3xl filter drop-shadow-md">{divisa.bandera}</span>
            ...
            <footer className="h-56 grid grid-cols-2 gap-8">
