// frontend/src/components/layoutsDirectorios/LayoutDirectorioHorizontal.jsx
...
export default function LayoutDirectorioHorizontal({ 
    config, datos, horaActual, isOnline, clima, itemActual, videoBlobUrl, FilaGaleria 
}) {
    const [pagina, setPagina] = useState(0);

    // ✅ PROTECCIÓN: Evita el error de 'map' y 'colores'
    if (!config || !config.colores || !datos || !horaActual) {
        return <div className="bg-black h-screen flex items-center justify-center text-white">Cargando...</div>;
    }
        ...
        <div className="flex flex-col h-screen w-screen overflow-hidden relative" style={{ backgroundColor: fondo }}>
            ...
            <main className="flex-1 px-10 py-4 flex flex-col gap-6 overflow-hidden">
                        ...
                        {visibles.map((e, i) => (
                            <div key={i} className="grid grid-cols-12 gap-4 items-center p-3 bg-white/5 border border-white/5 rounded-2xl shadow-lg animate-fade-in-up">
                                <div className="col-span-2 flex flex-col items-center justify-center" style={{ color: acento }}>
                                    ...
                                    <span className="text-[10px] opacity-40 uppercase">a</span>
                                    <span className="font-mono text-2xl font-bold">
                                ...
                                <div className="col-span-7 flex items-center gap-5 min-w-0">
                                    <div className="h-24 w-36 rounded-xl overflow-hidden shrink-0 border border-white/10">
                                        <img src={e.imagenes?.[0] || config.imagen_default} className="w-full h-full object-cover" />
                                    </div>
                                ...
                                <div className="col-span-3 flex items-center justify-end gap-4">
                                    <span className="text-sm font-bold uppercase bg-white/10 px-4 py-1.5 rounded-full" style={{ color: texto_reloj }}>{e.nombre_salon}</span>
                ...
                <div className="h-64 shrink-0 grid grid-cols-2 gap-6">
                    <div className="relative rounded-[2rem] overflow-hidden border border-white/10 bg-black/40">
                        <MediaRenderer url={itemActual} blobUrl={videoBlobUrl} className="absolute inset-0 w-full h-full object-cover" />
                    </div>
                    <div className="relative rounded-[2rem] overflow-hidden border border-white/10 bg-white/5 flex flex-col">
