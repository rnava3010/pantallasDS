            ...
            <div className="flex-1 flex flex-col bg-white/5 rounded-[4rem] border border-white/10 overflow-hidden shadow-2xl relative">
                ...
                <header className="p-10 flex justify-between items-center border-b border-white/10">
                    ...
                    <div className="text-right">
                        <span className="text-5xl font-black block" style={{ color: texto_reloj }}>
                            {horaActual?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                        <span className="text-sm font-bold opacity-50 uppercase tracking-widest" style={{ color: texto_reloj }}>
                    ...
                    {visibles.map((e, i) => (
                        <div key={i} className="flex items-center gap-8 border-b border-white/5 pb-6 last:border-0">
                            <div className="text-center min-w-[100px]">
                                ...
                                <span className="text-xs font-bold opacity-40 uppercase tracking-widest" style={{ color: texto_reloj }}>Inicia</span>
                            </div>
                            <div className="flex-1 min-w-0">
                                <h2 className="text-2xl font-black truncate" style={{ color: texto_evento }}>{e.nombre_evento}</h2>
                                <p className="text-sm opacity-50 uppercase font-bold" style={{ color: texto_reloj }}>{e.nombre_salon}</p>
                            </div>
